...
def handle_lu_action(payload: dict, api_data: VeraApiData) -> ResponsesResponse:
    ...
    set_state_variable_name = next(
        (key for key in params if key.lower().startswith("new")), "UserCodeName" if "UserCodeName" in params.keys() else "UserCode" if "UserCode" in params.keys() else None
    )
    state_variable_name = str(set_state_variable_name)[3:] if "UserCode" not in str(set_state_variable_name) else set_state_variable_name
